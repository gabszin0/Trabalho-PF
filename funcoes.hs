...
gerarRelatorio :: [Tarefa] -> String
gerarRelatorio tarefas =
  let total = length tarefas
      pendentes = length (filter ((== Pendente) . status) tarefas)
      concluidas = length (filter ((== Concluída) . status) tarefas)

      -- Conta quantas tarefas tem por categoria
      categoriasContagem = [ (cat, length (filter ((== cat) . categoria) tarefas)) 
                           | cat <- [Trabalho, Estudo, Pessoal, Outro] ]

      formatarCategoria (cat, qtd) = 
        let percentual = if total == 0 then 0 else fromIntegral qtd * 100 / fromIntegral total :: Double
        in printf "* %s: %d tarefa%s (%.1f%%)" 
                  (mostrarCategoria cat) 
                  qtd 
                  (if qtd == 1 then "" else "s") 
                  percentual
  ...
  in unlines $
    ...
    ] ++ map formatarCategoria categoriasContagem

-- Traduz o nome das categorias pra string (cores e plural)
mostrarCategoria :: Categoria -> String
mostrarCategoria Trabalho = "Trabalho"
mostrarCategoria Estudo   = "Estudos"
mostrarCategoria Pessoal  = "Pessoal"
mostrarCategoria Outro    = "Outro"
